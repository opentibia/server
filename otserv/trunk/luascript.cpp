...
int LuaScriptInterface::luaDoPlayerSetVocation(lua_State *L)
{
	...
	lua_pushnumber(L, (result ? LUA_NO_ERROR : LUA_ERROR));
...
int LuaScriptInterface::luaCanPlayerWearOutfit(lua_State *L)
{
	...
	Outfit outfit;
	if(Outfits::getInstance()->getOutfit(lookType, outfit)){
		lua_pushboolean(L, (player->canWearOutfit(outfit.outfitId, addon)? true : false));
		return 1;
	}
...
int LuaScriptInterface::luaDoPlayerSetVipLimit(lua_State *L)
{
	...
	Player* player = env->getPlayerByUID(cid);
	if(player){
		player->maxVipLimit = std::min(quantity, (uint32_t) 100);
		lua_pushnumber(L, LUA_NO_ERROR);
	}
	else{
		reportErrorFunc(getErrorDesc(LUA_ERROR_PLAYER_NOT_FOUND));
