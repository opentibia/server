function onUse(cid, item, frompos, item2, topos)
	...
	elseif isInArray(CLOSED_LOCKED_DOOR, item.itemid) == TRUE then
		if (getTileHouseInfo(topos) == FALSE) then
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "It is locked.")
		else		
				doTransformItem(item.uid, item.itemid + 1)
		end
	elseif (isInArray(OPENED_LOCKED_DOOR, item.itemid) == TRUE) then
		if ((getThingfromPos(doorPos).itemid > 0) or (getTilePzInfo(getThingfromPos(doorPos)) == TRUE) and (isPlayer(getThingfromPos(doorPos)))) then
			doPlayerSendCancel(cid, "Sorry, not possible.")
			return TRUE
		end
		doTransformItem(item.uid, item.itemid - 1)
